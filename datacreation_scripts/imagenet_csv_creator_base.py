...
save_dir = f"../keywords/imagenet"
os.makedirs(save_dir, exist_ok=True)
out = open(os.path.join(save_dir, "imagenet_base.csv"), "w")
...
list_folders = [name for name in os.listdir(imagenet_dir) if os.path.isdir(os.path.join(imagenet_dir, name))]
list_folders = sorted(list_folders)
print(os.listdir(imagenet_dir))
print(list_folders)
print(len(list_folders))
for i in trange(1000):
    folder_name, class_name = list_folders[i], openai_classnames[i]
    curr_path = os.path.join(f'../datasets/imagenet/train', folder_name)
    all_files = os.listdir(curr_path)
    tot_fils = 0
    for file in all_files:
        tot_fils += 1
        fp = os.path.join(curr_path[3:], file)
