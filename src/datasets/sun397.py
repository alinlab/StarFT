...
from torchvision.datasets import SUN397 as PyTorchSUN397
...
class SUN397:
    ...
    def __init__(self,
        ...
        self.dataset = PyTorchSUN397(
            root=self.location, 
            transform=preprocess, 
            download=True)

        train_size = int(len(self.dataset) * 0.4)
        validation_size = int(len(self.dataset) * 0.1)
        test_size = len(self.dataset) - train_size - validation_size

        self.train_dataset, self.validation_dataset, self.test_dataset = torch.utils.data.random_split(self.dataset, [train_size, validation_size, test_size])
        ...
        if self.test_subset == 'test':
            self.test_loader = torch.utils.data.DataLoader(
                self.test_dataset,
                batch_size=self.batch_size,
                shuffle=True,
                num_workers=self.num_workers)
        elif self.test_subset == 'val':
            self.test_loader = torch.utils.data.DataLoader(
                self.val_dataset,
                batch_size=self.batch_size,
                shuffle=True,
